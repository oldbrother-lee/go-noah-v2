...
func (e *MySQLExecutor) ExecuteDDLWithGhost() (ReturnData, error) {
	...
	if strings.Contains(e.Config.Hostname, "rds.aliyuncs.com") {
		...
		ghostCMDParts = append(ghostCMDParts, fmt.Sprintf("-assume-master-host=\"%s:%d\"", e.Config.Hostname, e.Config.Port))
	}
	...
	// 执行 gh-ost 命令（不使用超时，与老系统保持一致，改表系统不应该设置超时时间）
